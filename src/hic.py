...
class Hic:
    ...
    def __init__(self, cooler, chrom_num, square_side):
        self.cooler = cooler
    ...
    def set_sub_matrices(self):
        """
            Split the Hi-C matrix into X sub-matrices of size side*side.
    ...
    def plot_sub_matrices(self, color_map, output_path, index_list):
        ...
        fig.suptitle('True chr{} Hi-C sub-matrices'.format(self.chrom_num), fontsize=20)
        ...
        for axe, index in zip(axes.flat, self.sub_matrices[index_list, ..., 0]):
            ...
            i += 1
        plt.savefig('{}/submatrices_chr{}_true.png'.format(output_path, self.chrom_num))
        plt.close()
    ...
    def plot_matrix(self, color_map, output_path):
        ...
        axes.set_title('True chr{} Hi-C matrix'.format(self.chrom_num), fontsize=25)
        fig.savefig('{}/chr{}_true.png'.format(output_path, self.chrom_num))
        plt.close()
